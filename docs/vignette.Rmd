...
Such a plot can be obtained by looking at the model-based frequency-of-frequencies, which are available in closed form for the Pitman-Yor process [@Favaro_2013]. In particular, for a given $r = 1, \ldots, n$, the number of species appearing $r$ times is is equal to
...
expected_m_py(r = 1:10, n = sum(frequencies), alpha = coef(fit)[1], sigma =coef(fit)[2])
```
