...
```{r}
# Touring estimator of the sample coverage
...
The **Gini heterogeneity index** is a measure to quantify the amount of **biodiverity** present in the sample. In probabilistic terms, the Gini index is the *probability that two species are different when taken at random from the population*:
...
Species sampling models [@pitman_1996] are a class of **generative models** for species distributions which belong to the Bayesian nonprametric literature. The most common species sampling models are the Dirichlet process, `"DP"` [@Ferguson1973] and the Pitman-Yor process, `"PY"` [@Perman_1992]. For the purpose of this guide, we will only briefly discuss the basic properties of both models. For an extensive overview on how species sampling model work, see @DeBlasi_2015.
...
The parameters $\alpha$ and $\sigma$ govern the allocation scheme. In particular, it is easy to see that the Pitman-Yor process degenerates to the Dirichlet process when $\sigma = 0$. The interpretation of both equations is rather simple: the species observed at time $n+1$ can either be of a new type or of a previously observed one. In particular, the probability of observing an "old" species is increasing in the number of times that species has been observed in the past. The package `BNPvegan` contains a random sampler for both processes, which returns a vector of species frequencies. 
...
plot(fit, type = "freq")
```
Such a plot can be obtained by looking at the model-based frequency-of-frequencies, which are available in closed form for the Pitman-Yor process [@Favaro_2013]. In particular, for a given $\m = 1, \ldots, n$, the number of species appearing $m$ times is is equal to

$$
\mathbb{E}(M_m) = \frac{\alpha}{(\alpha)_n}\binom{n}{m}(1-\sigma)_{m-1}(\alpha + \sigma)_{n-m}.
$$
To obtain it, just run the function
```{r}
# Frequency-of-frequencies estimate under Pitman-Yor model
expected_m_py(m = 1:10, n = sum(frequencies), alpha = coef(fit)[1], sigma =coef(fit)[2])
```
