...
#'
#' @param x An object of class \code{\link[ssm]{ssm}}.
...
plot.DP <- function(x, type = "rarefaction", plot_sample = TRUE, m = NULL, verbose = TRUE, n_points = 100, ...) {
  ...
  if (type == "freq") {
    ...
    if(plot_sample == TRUE){
      ...
      p <- ggplot(df) +
        geom_point(aes(x = n, y = accum), shape = 1) +
        geom_line(aes(x = n, y = rar), color = "red", size = 0.9) +
        theme_bw() +
    ...
    if(plot_sample == TRUE){
      ...
      p <- ggplot(df) +
        geom_point(aes(x = n, y = accum), shape = 1, na.rm=TRUE) +
        geom_line(aes(x = n, y = curve), color = "red", size = 0.9) +
        theme_bw() +
